...
<template>
  ...
  >
    <NuxtLink class="self-center" to="/">
      <nuxt-img v-if="isDark" class="w-6" src="/logo-dark.png" alt="CP logo" />
      <nuxt-img v-else class="w-6" src="/logo.png" alt="CP logo" />
    </NuxtLink>
    <nav class="flex flex-row justify-between rounded-8 shadow-xl">
      <ul class="flex justify-center text-xs md:text-md font-bold py-1 px-2">
        ...
        <li
          class="flex ml-1 py-2 px-1 md:px-4 self-center rounded-8 hover:bg-opacity-70 dark:hover:bg-opacity-80 hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black"
        >
          <button
